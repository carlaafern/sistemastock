...
{
    public class IUnidadMedida:Intermediario
    {
	private string descripcionUnidadMed;
	private int codUnidadMed;
    ...
    {
        return (from p in dao.UnidadMedidas where p.descripcionUnidadMed == name select p).ToList() as List<T>; //.SingleOrDefault() as List<T>; //retorna un solo Producto
    }
    ...
    {
        return (from p in dao.UnidadMedidas select p).ToList() as List<T>;
    }
